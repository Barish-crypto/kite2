...
import asyncio, binascii, random, json, os, pytz, platform
...
class KiteAi:
            ...
            try:
                    ...
                    async with session.post(url=url, json=payload) as response:
                        ...
                        if result.get("errorId") != 0:
                            self.log(
                                ...
                                f"{Fore.RED + Style.BRIGHT} Capsolver Error - {result.get('errorDescription')} {Style.RESET_ALL}"
                            )
                        ...
                        for _ in range(30):
                            ...
                            async with session.post(url=res_url, json=res_payload) as res_response:
                                res_response.raise_for_status()
                                res_result = await res_response.json()

                                if res_result.get("errorId") == 0 and res_result.get("status") == "ready":
                                    recaptcha_token = res_result.get("solution", {}).get("gRecaptchaResponse")
                ...
                if attempt < retries - 1:
                    self.log(
                        f"{Fore.MAGENTA + Style.BRIGHT}  ● {Style.RESET_ALL}"
                        f"{Fore.BLUE + Style.BRIGHT}Message :{Style.RESET_ALL}"
                        f"{Fore.RED + Style.BRIGHT} Capsolver Attempt {attempt + 1} Failed - {str(e)} {Style.RESET_ALL}"
                    )
                    await asyncio.sleep(5)
                    continue
                return None
            ...
            while success_count < 30:
                ...
                if answer:
                    ...
                    if submit:
                        ...
                        self.log(
                            f"{Fore.BLUE + Style.BRIGHT}    Status  : {Style.RESET_ALL}"
                            f"{Fore.GREEN + Style.BRIGHT}Receipt Submitted Successfully{Style.RESET_ALL}"
                        )
    ...
    async def main(self):
        try:
            ...
            captcha_key = self.load_capsolver_key()
            if captcha_key:
